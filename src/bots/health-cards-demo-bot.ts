...
const _RESOURCE_TYPES_VALUE_SET_CANONICAL_URL =
...
export async function handler(
  ...
  // Ensure the patient is requesting a health card for themselves
  if (event.requester?.id && event.requester.id !== patientReference.id) {
    throw new Error(
      "You can only request a health card logged in as the patient"
    );
  }
  ...
  const credentialTypes = credentialTypeParameters
    .map((p) => p.valueUri)
    .filter(Boolean);
  const resourceTypesVS = await medplum.valueSetExpand({
    url: _RESOURCE_TYPES_VALUE_SET_CANONICAL_URL,
  });
  const _allowedResourceTypes = new Set(
    resourceTypesVS.expansion?.contains?.map((c) => c.code) ?? []
  );
  ...
  const since = parameters.parameter?.find(
    (p) => p.name === "_since"
  )?.valueDateTime;
  if (since) {
    const iso8601Regex =
  ...
  const sanitizedPatient = sanitizePatient(
    patient as Patient,
    includeIdentityClaims as string[]
  );
  ...
  if (credentialValueSets.length > 0) {
    ...
    filteredResources = filterResourcesByValueSets(
      collectedResources,
      expandedVSets
    );
  }
  ...
  const bundle: Bundle = {
    ...
    entry: [
      ...(patient
        ? [{ fullUrl: `Patient/${patient.id}`, resource: sanitizedPatient }]
        : []),
  ...
  const issuer = new SHCIssuer({
    ...
    privateKey: JSON.parse(
      event.secrets.HEALTH_CARD_PRIVATE_KEY?.valueString as string
    ),
    publicKey: JSON.parse(
      event.secrets.HEALTH_CARD_PUBLIC_KEY?.valueString as string
    ),
